...
const DragAndDrop: React.FC = () => {
    ...
    try {
      ...
      for (let page = 1; page <= totalPages; page++) {
        ...
        const userNames = todos.map(
          (item: { UserName: any }, index: number) => ({
            id: (page - 1) * 10 + index,
            name: item.UserName,
          })
        );
  ...
  const handleDrop = async (e: any) => {
    ...
    try {
      await axios.post("http://localhost:5567/primeuser/create", {
        UserName: droppedItem.name,
      });
      console.log(
        `Record with name: "${droppedItem.name}" created successfully.`,
        "111"
      );
  ...
  const showAll = () => {
    axios
      .post("http://localhost:5567/primeuser/getData")
      .then((res) => {
  ...
  const handleDeleteButton = (Id: number) => {
    axios
      .post(`http://localhost:5567/primeuser/deleteUser/${Id}`)
      .then((res: { data: any }) => {
        console.log(res.data);
        showAll();
      })
